...
type ProductItem = CategoryCourseItem;
...
export default function RiskettanPage() {
  ...
  // State for schedule API integration
  const [riskOneSlots, setRiskOneSlots] = useState<ProductItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  
  // State for page content (from Riskettan CMS)
  const [pageContent, setPageContent] = useState<RiskettanContentData | null>(null);
  ...
  useEffect(() => {
    const loadPageContent = async () => {
      try {
        ...
        console.log(`🌐 Loading Riskettan content for language: ${language}`);
        
        const content = await riskettanContentService.getRiskettanContent(language);
        
        if (content) {
          setPageContent(content);
          console.log(`✅ Riskettan ${language.toUpperCase()} content loaded successfully`);
        } else {
          console.warn(`⚠️ No content found for language: ${language}`);
        }
      } catch (error) {
    ...
    <>
      ...
      {contentLoading ? (
        ...
        <>
          ...
          <div className="bg-[#F7FAFF] py-[56px] md:py-[120px] px-4">
            <div className="w-full xl:w-[1320px] mx-auto">
              <h1 className="text-24 sm:text-56 font-bold  text-[#1D1F2C] leading-[140%] text-center pb-5">
                {pageContent?.pageContent?.title || 'Risk 1 Course'}
              </h1>
          ...
          {/* Course Details Section with Dynamic Content */}
          <div className=" bg-white py-[56px] xl:py-[120px] px-4 ">
            <div className="w-full xl:w-[1320px] mx-auto">
              <h1 className="text-[24px] sm:text-35 font-[600]  text-[#1D1F2C]  pb-5 tracking-[0.5%]">
                {pageContent?.pageContent?.mainTitle || 'Complete Risk 1 Training Program'}
              </h1>
              <p className="text-20 sm:text-30 font-[500]  text-[#1D1F2C] leading-[100%]  pb-3">
                {pageContent?.pageContent?.subtitle || 'Professional Driving Education'}
              </p>
              <p className="text-16 font-[400]  text-[#000000] leading-[140%] tracking-[0.5%]   w-11/12 pb-10">
                {pageContent?.pageContent?.introText || 'Our comprehensive Risk 1 course covers all fundamental aspects of safe driving'}
              </p>
              ...
              <div className="flex justify-between items-center pb-12 flex-col-reverse md:flex-row">
                <div className="w-full md:w-[633px]">
                  <h3 className="text-20 sm:text-32 font-medium  my-4">
                    {pageContent?.whyRisk1?.title || 'Why Choose Risk 1 Course'}
                  </h3>
              ...
              <h2 className="text-20 sm:text-32 font-medium mb-3 md:mb-6 text-[#1D1F2C]">
                {pageContent?.courseContent?.title || 'Course Content Overview'}
              </h2>
              <ul className="space-y-2 text-16 md:text-18 font-medium text-[#4A4C56] mb-8">
                {(pageContent?.courseContent?.items || []).map((item, index) => (
                  <li key={index} className="flex items-center ">
                    ...
                    <span>{item?.title || 'Course item'}</span>
                  </li>
                ))}
              </ul>
